      ...
      <div className="w-full max-w-lg mx-auto space-y-8 flex-1 flex flex-col justify-center">
          ...
          <div className="relative w-full max-w-sm">
            ...
            <div className="absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2 flex items-center">
              <div className="flex border rounded-full overflow-hidden">
                <button onClick={() => setUnit('km')} className={`px-2 sm:px-3 py-1 text-xs sm:text-sm font-medium transition-colors ${unit === 'km' ? 'bg-beige text-beige-foreground' : 'bg-background hover:bg-beige-hover'}`}>
                  KM
                </button>
                <button onClick={() => setUnit('miles')} className={`px-2 sm:px-3 py-1 text-xs sm:text-sm font-medium transition-colors border-l ${unit === 'miles' ? 'bg-beige text-beige-foreground' : 'bg-background hover:bg-beige-hover'}`}>
                  MILES
                </button>
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Button variant={locationMethod === 'current' ? 'default' : 'outline'} onClick={handleUseCurrentLocation} className="w-full sm:w-44 px-4 py-2.5 sm:py-3 h-10 sm:h-12 rounded-full border-2 font-semibold uppercase tracking-wide text-xs sm:text-sm" disabled={isGenerating}>
              Current Location
            </Button>

            <span className="text-xs text-muted-foreground uppercase tracking-wider font-light">or</span>

            <Button variant={locationMethod === 'map' ? 'default' : 'outline'} onClick={handleChooseOnMap} className="w-full sm:w-44 px-4 py-2.5 sm:py-3 h-10 sm:h-12 rounded-full border-2 font-semibold uppercase tracking-wide text-xs sm:text-sm" disabled={isGenerating}>
              Choose on Map
            </Button>
          </div>
        </div>
        ...
        {/* Generate Button */}
        <div className="flex justify-center">
          <Button onClick={handleGenerate} disabled={isGenerating || !distance || !selectedLocation} size="lg" className="w-full sm:w-auto px-8 py-3 h-12 sm:h-14 rounded-full uppercase tracking-wide text-sm sm:text-base bg-[#f7eeda] font-semibold text-black">
